...
<main>
    <div class="py-4 mx-auto w-full md:py-4">
        ...
        <div class="flex px-6 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Edit Quick Link</h1>
                <p class="text-gray-600 dark:text-gray-400">Update quick link details for your website.</p>
            </div>
        </div>

        <!-- Form Section -->
        <div x-data="slugGenerator('{{ old('name', $quickLink->name ?? '') }}', '{{ old('slug', $quickLink->slug ?? '') }}')" class="border-gray-100 p-5 dark:border-gray-800 sm:p-6">
            <div class="rounded-2xl px-6 pb-8 pt-4 border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                <form action="{{ route('be.quick-link.update', $quickLink->slug) }}" method="POST">
                    @csrf
                    @method('PUT')
            ...
            return {
                ...
                updateSlug() {
                    if (this.name.length > 0) {
                        fetch("{{ route('be.quick-link.generate.slug') }}", {
                            method: 'POST',
                            headers: {
