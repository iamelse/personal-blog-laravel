...
<main>
    <div class="py-4 mx-auto w-full md:py-4">
        ...
        <div class="flex px-6 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Edit Social Media</h1>
                <p class="text-gray-600 dark:text-gray-400">Update social media details.</p>
            </div>
        </div>

        <!-- Form Section -->
        <div x-data="slugGenerator('{{ old('name', $socialMedia->name ?? '') }}', '{{ old('slug', $socialMedia->slug ?? '') }}')" class="border-gray-100 p-5 dark:border-gray-800 sm:p-6">
            <div class="rounded-2xl px-6 pb-8 pt-4 border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                <form action="{{ route('be.social-media.update', $socialMedia->slug) }}" method="POST">
                    @csrf
                    @method('PUT')
            ...
            <div class="rounded-2xl px-6 pb-6 pt-6 border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] text-gray-700 dark:text-gray-300">
                <div class="">
                    <p class="mb-2">
                        ...
                        <a href="https://boxicons.com/" target="_blank" class="text-blue-600 dark:text-blue-400 underline">Boxicons</a> and use the font class.
                    </p>
                </div>
            </div>
        </div>
